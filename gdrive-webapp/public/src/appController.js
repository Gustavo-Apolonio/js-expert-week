export default class AppController {
    ...
    async onProgress({ proccessedAlready, filename }) {
        const file = this.uploadingFiles.get(filename)
        const alreadyPercentage = Math.ceil(proccessedAlready / file.size * 100)
        this.updateProgress(file, alreadyPercentage)
        if (alreadyPercentage < 98) return
